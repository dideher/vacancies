...
class Covid19EntryUpdateForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not is_empty(teacher_name) and is_empty(teacher_surname):
            self.add_error('teacher_surname',
                           _('Αφού δηλώσατε όνομα, τότε πρέπει να δηλώσετε ΚΑΙ το επώνυμο του εκπαιδευτικού'))

        if is_empty(teacher_name) and not is_empty(teacher_surname):
            self.add_error('teacher_name',
                           _('Αφού δηλώσατε επώνυμο, τότε πρέπει να δηλώσετε ΚΑΙ το όνομα του εκπαιδευτικού'))

        if illness_started and illness_end_estimation and illness_started >= illness_end_estimation:
            self.add_error('illness_end_estimation',
                           _('Η ημ/νία πιθανής επιστροφής είναι προγενέστερη ή ίση της ημ/νιας νόσησης'))
